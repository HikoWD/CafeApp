...
@Singleton
class LoadMenuItemsUseCase @Inject constructor(private val menuItemRepository: MenuItemRepository) {
    suspend operator fun invoke(): CompletableResult = withContext(Dispatchers.IO) {
        return@withContext when (val result = menuItemRepository.loadMenuItems()) {
