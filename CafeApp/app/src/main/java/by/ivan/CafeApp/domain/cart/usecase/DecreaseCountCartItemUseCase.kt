...
@ViewModelScoped
class DecreaseCountCartItemUseCase @Inject constructor(private val cartItemRepository: CartItemRepository) {
    suspend operator fun invoke(menuItem: MenuItem) = withContext(Dispatchers.IO) {
        cartItemRepository.removeItemOrDecreaseCount(
            cartItemLocalModel = menuItem.toCartItemLocalModel(
                count = 1
            )
        )
    }
